...
class noror:
    ...
    def forecast_mod(self,init,n_steps):
        #apply the dimensionality reduction to the initital conditions
        if self.reduce_dim == True:
            reduced_init = self.dim_reducer.reduce(init,self.rdim)[:self.VAR_l]
        else:
            reduced_init = init[:self.VAR_l]
        ...
        #setup numpy array for the buffer
        buff = np.zeros((self.VAR_l,reduced_init.shape[1]))
        #build initial condition for the auto predictions
        m_buff = 0
        if (self.full_hist == True):
            buff[:self.VAR_l] = reduced_init[:self.VAR_l]
        else:
            for j in range(buff.shape[0]):
                buff[j] = reduced_init[0]
        
        #let the machine predict the dynamics
        for j in range(j_start,pred.shape[0]):
        
            VAR_vec = np.concatenate([buff[(m_buff+m)%self.VAR_l] for m in range(self.VAR_l)]).reshape(1,-1)
    ...
    def forecast_mod2(self,init,n_steps):
        #apply the dimensionality reduction to the initital conditions
        if self.reduce_dim == True:
            reduced_init = self.dim_reducer.reduce(init,self.rdim)[:self.VAR_l]
        else:
        ...
        #setup numpy array for the auto prediction
        pred = np.zeros((n_steps,init.shape[1]))
        #copy initial conditions
        if (self.full_hist == False):
            j_start = 1
            pred[0] = init[0]
            ...
            j_start = self.VAR_l
            pred[:self.VAR_l] = init[:self.VAR_l]
        ...
        for j in range(j_start,pred.shape[0]):
            ...
            #predict the next step
            pred_diff = transform @ self.w
            
            pred_step = pred[j-1] + pred_diff
            
            pred[j] = pred_step
            
            
            if self.reduce_dim == True:
                pred_step = self.dim_reducer.reduce(pred_step,self.rdim)
                
            #apply data/feature scaling
            if self.standardize:
                pred_step = self.scaler.transform(pred_step)
            
            buff[m_buff] = pred_step
            m_buff = (m_buff+1)%self.VAR_l
